...
2. **Fork 本仓库** `mmlong818/catchat` 到你自己的账号（点仓库右上角 Fork 按钮）
3. 也可以直接用本项目已经部署好的公共信令 `wss://catchat-signal.onrender.com`（默认值就是这个），跳过 2.2/2.3。⚠️ 这是社区共享实例，Render 免费层 15 分钟休眠，建议自部署
...
| **Root Directory** | `signaling-server` ← **必填**，告诉 Render 只构建这个子目录 |
